...
export function Approvals() {
  ...
  const selGroups = fig
    ? [
          ...
          items: [
            {
              ...
              value: fig.present,
              fg: fig.present < 12 ? colors.red : colors.ink,
            },
        ...
        },
        {
          label: "Maturity",
          items: [
            { label: "100% paid in", value: fig.paidIn },
            { label: "Baptism in the Holy Ghost", value: fig.baptism },
          ],
        },
        {
          label: "Ministry",
          items: [
            ...
            {
              label:
                "Cell leader visitation (Phone calls, Physical visitation, Text or WhatsApp Message)",
              value: fig.clVisit,
            },
          ],
        },
  ...
  const apprApprove = () => {
    if (!selNode) return;
    set((s) => ({
      apprDone: [...s.apprDone, selNode.code],
      apprLog: { ...s.apprLog, [selNode.code]: "approved" },
      apprSel: 0,
    }));
  };
  const apprReject = () => {
    ...
    set((s) => ({
      apprDone: [...s.apprDone, selNode.code],
      apprLog: { ...s.apprLog, [selNode.code]: "sent-back" },
      apprSel: 0,
    }));
  };
  const apprCleanCodes = pend
    .filter((p) => figFor(p).present >= 12)
    .map((p) => p.code);
  const apprAll = () => {
    set((s) => ({
      apprDone: [
        ...s.apprDone,
        ...apprCleanCodes.filter((c) => s.apprDone.indexOf(c) === -1),
      ],
      apprLog: apprCleanCodes.reduce(
        (acc, c) => ({ ...acc, [c]: "approved" as const }),
        { ...state.apprLog },
      ),
      apprSel: 0,
    }));
  ...
  const selComment = selNode
    ? APPROVAL_COMMENTS[pend.indexOf(selNode) % APPROVAL_COMMENTS.length]
    : "";
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 13, color: colors.muted, marginTop: 5 }}>
              {apprOpen.length} report{apprOpen.length === 1 ? "" : "s"} waiting
              on you. Anything not acted on in 36 hours escalates to the Area
              Coordinator.
            </div>
          </div>
          <Button
      ...
      >
        <Card style={{ flex: "1 1 290px", maxWidth: 360, overflow: "hidden" }}>
          ...
          {apprList.map((a, i) => (
            <div
              ...
              style={{
                ...
                gap: 12,
                alignItems: "center",
                borderLeft: `3px solid ${a.on ? colors.red : "transparent"}`,
                background: a.on ? colors.fieldBg : "#fff",
              }}
            >
          ...
          {apprOpen.length === 0 && (
            ...
            >
              Queue cleared. Nothing is waiting on you.
            </div>
          )}
        </Card>
          ...
          <Card style={{ flex: "2 1 400px", minWidth: 0, overflow: "hidden" }}>
            ...
            >
              <div>
                ...
                <div
                  style={{ fontSize: 12.5, color: colors.faint, marginTop: 3 }}
                >
                  Cell Leader {selNode.leader} · {selNode.code} · submitted via{" "}
                  {selNode.channel} at {selNode.time}
                </div>
              </div>
              <span
                style={{
            ...
            {selFlagged && (
              ...
              >
                Attendance of {fig!.present} is more than 30% below this
                cell&rsquo;s 8-week average. Confirm with the leader before
                approving.
              </div>
            )}
            <div style={{ padding: "20px 24px 24px" }}>
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            fontFamily: mono,
                            marginTop: 6,
                            color: "fg" in it ? it.fg : colors.ink,
                          }}
                        >
                          {it.value}
                        </div>
              ...
              >
                ...
                <Button variant="ghost" padding="13px 12px" fontSize={14}>
                  Message leader
                </Button>
              </div>
            </div>
